{ *********************************************************************** }
{                                                                         }
{ PM Code Works Windows Utilities Unit v1.4                               }
{                                                                         }
...
uses
  Windows, Classes, SysUtils, TLHelp32, Registry, ShellAPI;
...
type
  ...
  public
    ...
    class function ShowAddRegistryDialog(const FRegFilePath: string): Boolean;
...
{ public TWinUtils.CheckWindows

  Returns if current Windows version is newer than Windows XP. }

class function TWinUtils.CheckWindows: Boolean;
begin
  result := GetWinVersion()[1] in ['V','7','8'];
end;

{ public TWinUtils.CreateTempDir

  Creates an new folder in the temporay directory. }

class function TWinUtils.CreateTempDir(const AFolderName: string): Boolean;
begin
  result := ForceDirectories(GetTempDir() + AFolderName);
end;

{ public TWinUtils.ExecuteFile

  Executes an executable file using ShellExecute. }

class function TWinUtils.ExecuteFile(const AFilePath: string): Boolean;
begin
  result := (ShellExecute(0, 'open', PChar(AFilePath), nil, nil, SW_SHOWNORMAL) >= 32);
end;

{ public TWinUtils.ExitWindows
...
{ public TWinUtils.GetBuildNumber

  Restarts the explorer task of Windows. }

class function TWinUtils.ExplorerReboot(): Boolean;
begin
...
{ public TWinUtils.GetTempDir

  Returns path to the temporary directory of Windows. }

class function TWinUtils.GetTempDir(): string;
begin
  result := SysUtils.GetEnvironmentVariable('temp');
end;

{ public TWinUtils.GetWinDir
...
{ public TWinUtils.GetWinVersion

  Returns used Windows version with optional information about installed
  service packs. }

class function TWinUtils.GetWinVersion(AShowServicePack: Boolean = False): string;
begin
  result := 'unbekannt';
...
{ public TWinUtils.OpenUrl

  Opens a given URL in the default web browser. }

class function TWinUtils.OpenUrl(const AUrl: string): Boolean;
begin
  result := (ShellExecute(0, 'open', PChar(AUrl), nil, nil, SW_SHOWNORMAL) >= 32);
end;

{ public TWinUtils.PMCertExists

  Returns if the PM Code Works certificate is already installed. }
  
class function TWinUtils.PMCertExists(): Boolean;
var
  reg: TRegistry;
  Exists: Boolean;
...
{ public TWinUtils.ShowAddRegistryDialog

  Shows an dialog where user has the choice to add a *.reg file.  }

class function TWinUtils.ShowAddRegistryDialog(const FRegFilePath: string): Boolean;
begin
  result := (ShellExecute(0, nil, PChar('regedit'), PChar(ExtractFileName(FRegFilePath)), 
               PChar(ExtractFilePath(FRegFilePath)), SW_SHOWNORMAL) >= 32); 
end;

{ public TWinUtils.StrToHKey

  Converts short HKEY string into real HKEY type. } 

class function TWinUtils.StrToHKey(AMainKey: string): HKEY;
begin
